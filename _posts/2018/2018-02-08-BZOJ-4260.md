---
layout: post
title: BZOJ - 4260 01字典树+前后缀
categories: [ICPC]
description: 给定$$a[1...n]$$,求$$(a_{i}⊕a_{i+1}⊕\cdots ⊕a_{j})+(a_{p}⊕a_{p+1}⊕\cdots⊕a_{q})$$的最大值,其中$$1≤i≤j<p≤q≤n$$
---

题意:给定$$a[1...n]$$,求$$(a_{i}⊕a_{i+1}⊕\cdots ⊕a_{j})+(a_{p}⊕a_{p+1}⊕\cdots⊕a_{q})$$的最大值,其中$$1≤i≤j<p≤q≤n$$
