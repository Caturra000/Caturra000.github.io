---
layout: post
title: Codeforces - 527C 平衡树维护几何
categories: [ICPC]
description: 题意:给定一个矩形$$W*H$$,一共$$n$$次切割操作(水平/垂直),求每次操作后得出的最大面积
---

题意:给定一个矩形$$W*H$$,一共$$n$$次切割操作(水平/垂直),求每次操作后得出的最大面积
<!--more-->


随机按tag扫CF题目找到的题,可以分别用平衡树维护割边的位置和长度($$x/y$$各两个)
    ...
    while(cin>>w>>h>>n){
        ...
        rep(i,1,n){
            ...
            if(str[0]=='H'){
                ...
                it1--;
                ll yy1=*it1,yy2=*it2;
                st[1].insert(y); mst[1].erase(mst[1].find(yy2-yy1)); //只能往迭代器删.. 
